        ...
        if self.share_weights:
            self.W = nn.Parameter(
                nn.init.xavier_normal_(
                    t.empty(1, input_dim_capsule, self.num_capsule * self.dim_capsule)
                )
            )
        else:
            self.W = nn.Parameter(
    ...
    def forward(self, x):
        ...
        for i in range(self.routings):
            ...
            outputs = self.activation(
                t.einsum("bij,bijk->bik", (c, u_hat_vecs))
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(
            hidden_size * 8 + 3, fc_layer1
        )  # 643:80 - 483:60 - 323:40
        self.relu = nn.ReLU()
...
print(
    "All \t loss={:.4f} \t val_loss={:.4f} \t ".format(
